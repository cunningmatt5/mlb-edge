...
def score_team_totals(game: dict, cache: dict) -> list[dict]:
    ...
    for offense_side, sp_side in [("home", "away"), ("away", "home")]:
        ...
        # Opposing SP run-suppression quality
        ...
        # OVER: strong offense vs weak SP in hitter-friendly park
        over_raw = weighted_avg([
            (offense_s,              0.45),
            (1.0 - sp_suppress,      0.35),
            (park_s,                 0.20),
        ])
        over_signal = round(over_raw * 10, 1)

        # UNDER: weak offense vs elite SP in pitcher-friendly park
